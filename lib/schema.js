...
var TimeSeriesSchema = function(args) {
  ...
  */
  ...
  function getUpdates(timestamp, value, metadata){
    ...
    var updates = {}
    if( options.interval < 1 ) {
      updates['milliseconds.'+timestamp.getHours()+'.m.'+timestamp.getMinutes()+'.s.'+timestamp.getSeconds()+'.ms.'+timestamp.getMilliseconds()+'.value']=value;
      ...
      updates['seconds.'+timestamp.getHours() +'.m.'+ timestamp.getMinutes() +'.s.'+ timestamp.getSeconds()+'.value']=value;
      if( metadata ) updates['seconds.'+ timestamp.getHours() +'.m.'+ timestamp.getMinutes() +'.s.'+ timestamp.getSeconds() +'.metadata']=metadata;
    } else if( options.interval < 3600 ){
      updates['minutes.'+timestamp.getHours() +'.m.'+ timestamp.getMinutes()+'.value']=value;
      if( metadata ) updates['minutes.'+ timestamp.getHours() +'.m.'+ timestamp.getMinutes() +'.metadata']=metadata;
